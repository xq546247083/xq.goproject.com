//Package webServer 此文件为：改造http.FileServer
package webServer
...
func dirList(w http.ResponseWriter, f http.File) {
	...
	if err != nil {
		...
		// Issue 12438.
		http.Error(w, "Error reading directory", StatusInternalServerError)
		return
	}
...
func checkIfMatch(w http.ResponseWriter, r *http.Request) condResult {
	...
	for {
		...
		if etagStrongMatch(etag, headerGetVal(w.Header(), "Etag")) {
			return condTrue
		}
		im = remain
	}
...
func checkIfNoneMatch(w http.ResponseWriter, r *http.Request) condResult {
	inm := headerGetVal(r.Header, "If-None-Match")
	if inm == "" {
		return condNone
	}
	buf := inm
	for {
		...
		if etagWeakMatch(etag, headerGetVal(w.Header(), "Etag")) {
			return condFalse
		}
		buf = remain
	}
...
func checkIfRange(w http.ResponseWriter, r *http.Request, modtime time.Time) condResult {
	...
	ir := headerGetVal(r.Header, "If-Range")
	if ir == "" {
		return condNone
	}
	etag, _ := scanETag(ir)
...
func checkPreconditions(w http.ResponseWriter, r *http.Request, modtime time.Time) (done bool, rangeHeader string) {
	...
	rangeHeader = headerGetVal(r.Header, "Range")
	if rangeHeader != "" {
...
func ServeFile(w http.ResponseWriter, r *http.Request, name string) {
	if containsDotDot(r.URL.Path) {
		...
		http.Error(w, "invalid URL path", StatusBadRequest)
		return
	}
...
func headerGetVal(h http.Header, key string) string {
	if v := h[key]; len(v) > 0 {
		return v[0]
	}
	return ""
